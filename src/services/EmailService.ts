 ...
 *
 * Delivers outbound emails via Resend (primary) with a console fallback.
 * No new package required — uses native fetch which Node 18+ provides.
 *
 * Configuration:
 *   RESEND_API_KEY   — Resend (https://resend.com) — primary
 *   SENDGRID_API_KEY — SendGrid fallback
 *   EMAIL_FROM       — Sender address (default: atlas@yourdomain.com)
 *
 * If neither key is configured, emails are logged to console only
