...
// ─── Resolver ─────────────────────────────────────────────────────────────────
// Finds org + executive by name fragment, returns typed executive instance.

async function resolveExec(nameFragment: string) {
  ...
  const exec = await prisma.aIExecutive.findFirst({
    where: { organizationId: org.id, name: { contains: nameFragment, mode: 'insensitive' } },
  });
...
router.post('/atlas/strategy-session', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Atlas');
...
router.post('/zephyr/qualify-lead', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Zephyr');
...
router.post('/aurelia/draft-proposal', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Aurelia');
...
router.post('/aria/plan-campaign', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Aria');
...
router.post('/lyra/health-score', wrap(async (req, res) => {
  const { leadId } = req.body;
  if (!leadId) return res.status(400).json({ error: 'leadId required' });
  const { org, exec } = await resolveExec('Lyra');
...
router.post('/sage/job-description', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Sage');
...
router.post('/orion/audit-process', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Orion');
...
router.post('/lexis/draft-nda', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Lexis');
...
router.post('/forge/generate-code', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Forge');
...
router.get('/iris/intelligence-report', wrap(async (req, res) => {
  ...
  const { org, exec } = await resolveExec('Iris');
